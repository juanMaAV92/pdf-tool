...
def output_path_for_images(input_path: Path, out_dir: Path | None = None) -> Path:
...
def _convert_one(
    input_path: Path, progress: Progress, out_dir: Path | None = None
) -> tuple[Path | None, str]:
...
def images_to_pdf(
    inputs: list[Path], params: ImagesToPdfParams, progress: Progress = _noop
) -> ToolResult:
    if not inputs:
    ...
    if not outputs:
        raise ValueError(
            labels[0] if total == 1 else "Ninguna imagen pudo convertirse."
        )

    if total == 1:
        return ToolResult(
            outputs=outputs, summary=f"Imagen convertida → {outputs[0].name}"
        )
